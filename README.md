...
| Version | Date | Change |
|---------|------|--------|
| v37.19 | 2026-09-02 | **Stage error stops averaging over the observations the learner rejects (TODO #27) — MINOR. Moves a published number; no model, learning, correction or estimate change.** `sumAbsStageError`/`avgStageError` accumulated *outside* the `!isHardFlagged` gate while the `stage_*` bin write sits *inside* it, so the headline stage error averaged over hard-flagged (physically implausible) observations that the correction bins themselves discard. Live at the time of the fix: **n=313 in the metadata vs 295 in the bins — 18 outliers inflating a published number.** This is the same defect v32.3 fixed for `avgErrorPercent`; it was simply never applied to the stage series. **Fix:** a clean series (`stageObsClean`, `sumAbsStageErrorClean`, `avgStageErrorClean`) accumulates under the *same* gate as the stage bin write, so `stageObsClean` equals the sum of the `stage_*` bins by construction — and that equality is now a live cross-check: if the two diverge, a bin write is failing. The Learning-tab "Stage Error" row reads the clean series. **The legacy fields are frozen, not reset and not deleted.** `sumAbsStageError` is a cumulative sum rather than an EMA, so the contamination never washes out, and it cannot be repaired in place — the bins store *signed* `sumError`, not sum\|error\|, so the clean history is unrecoverable. Freezing follows the v37.16 precedent for the retired NWS counters: the old number stays visible after a dot, labelled frozen, as an audit trail, and nothing silently rewrites history. `stageValidations` keeps its original semantics (every validation that had a usable stage pair, hard-flagged or not) because it is correct as a *count* — only the average was polluted — and because the v37.17 diagnostics derive the null-stage total from it. **Expect the displayed stage error to move** when the clean series builds; until then the row reads "clean series building" beside the frozen value. Both branches locked by tests (a hard-flagged stage observation must not enter the clean series; the legacy fields must never be written again). 768 → 770 tests. |
| v37.18 | 2026-09-02 | **Dead ice-cleanup reset removed + two correctness fixes (TODO #28, #29) — MINOR. No model, learning, correction or estimate change.** Both defects were surfaced by the adversarial verification of v37.17, not by v37.17 itself. **(1) `resetLowFlowBins` deleted outright** (server action, client function, button, binding — 79 + 33 lines). The v24 ice-cleanup action was confirmed dead by the user and was *actively harmful* while it existed: it deleted only 12 of the 28 correction bins but upserted a `newMeta` literal that **replaces the whole metadata jsonb**, so the surviving high-flow bins kept their observation counts while every counter restarted at 0 — permanently desynchronising the diagnostics panel from the bins, with no way to tell after the fact. A stale cached client that still POSTs the action now falls through to the handler's `result = { success: false }` default and reports a failed reset: it cannot delete anything. A structural guard test locks the removal (same pattern as the v37.1 System-1 retirement). **(2) `resetGFLearning` now actually keeps what its `// Keep health stats` comment promised.** Because that upsert replaces the whole jsonb, every field not named in the literal is *destroyed*, and `binWriteSuccesses`/`binWriteFailures`/`lastBinError` were not named — so a learning reset silently zeroed the bin-write health counters, which are the entire point of the v37.7 diagnostics panel. They are operational telemetry about whether writes are landing, not learning state, so they now survive; the learning stats are still explicitly zeroed, and the stage series still resets because the reset does delete the `stage_*` bins. **(3) `errorStage` used truthiness** — `(predictedStage && actualStage)` — so a legitimate **0.00 ft** reading was discarded as a missing measurement, and the new v37.17 `stageSkipped` counter would have silently absorbed it. Now `Number.isFinite` on both, which is deliberately stricter than the `!= null` the fix first used: `actualStage = lf.h` comes from parsed USGS text, so **NaN is reachable**, the old truthiness test at least rejected it, and `!= null` would not have — a NaN would poison `sumAbsStageError` irrecoverably. Both branches are locked by tests (0.00 ft counts as an observation; NaN counts as a skip). Not reachable at Little Falls today (stage never approaches 0), but it was the wrong predicate. 760 → 768 tests. |
