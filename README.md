...
**Current Version**: v37.16 (August 2026)
...
*Last updated: 2026-08-28 (v37.16 — forecast validation clock: a forecast for wall-clock t predicts flow at GREAT FALLS at t, but the validator scored it against Little Falls at t, omitting the GF→LF travel time (~9h at 2,800 cfs, up to 16.95h at the discharge floor — larger than the +6h horizon itself at summer flows). Forecasts now validate when the water actually arrives, as CLAUDE.md and tech-appendix §8.6 already specified. The two NWS baselines were retired: on the model's own clock they are the model plus a constant, so the "vs NWS" delta could never carry skill — persistence (observed LF) is now the displayed comparison. Metrics/display only; the estimate and EMA learning are untouched. Accumulated forecast accuracy needs a reset to be interpretable — TODO #23.)*
