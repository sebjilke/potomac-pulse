...
**Current Version**: v37.17 (September 2026)
...
*Last updated: 2026-09-02 (v37.17 — stage-skip counter. The `stage_*` bins only receive a validation when a predicted AND actual gauge height both exist, and that write sits inside the hard-flag gate; when either is missing the CFS bin learns alone and nothing counted the difference (`binWriteFailures` stays 0 — a null stage is not a write failure). Live: the stage series is 12 observations behind the CFS series. Added a `stageSkipped` counter (guarded on `!isHardFlagged`, mutation-tested) plus a Learning-tab row showing it beside the derived null-stage total. Diagnostics only — `buildCorrectionBins` skips `stage_*` and the GF estimate never reads them.)*
