...
**Current Version**: v37.18 (September 2026)
...
*Last updated: 2026-09-02 (v37.18 — removed the dead `resetLowFlowBins` ice-cleanup action, which deleted 12 of 28 bins while zeroing every counter and so permanently desynchronised the diagnostics; made `resetGFLearning` actually preserve the bin-write health counters its comment claimed to keep (the metadata upsert replaces the whole jsonb, so anything unnamed is destroyed); and replaced the `errorStage` truthiness test with `Number.isFinite`, so a real 0.00 ft stage counts as a measurement while NaN is still rejected. All three found by adversarial verification of v37.17. No model, learning or estimate change.)*
