...
| Version | Date | Change |
|---------|------|--------|
| v37.19 | 2026-09-02 | **Stage error stops averaging over the observations the learner rejects (TODO #27) — MINOR. Moves a published number; no model, learning, correction or estimate change.** `sumAbsStageError`/`avgStageError` accumulated *outside* the `!isHardFlagged` gate while the `stage_*` bin write sits *inside* it, so the headline stage error averaged over hard-flagged (physically implausible) observations that the correction bins themselves discard. Live at the time of the fix: **n=313 in the metadata vs 295 in the bins — 18 outliers inflating a published number.** Same defect v32.3 fixed for `avgErrorPercent`; never applied to the stage series. **Fix:** a clean series (`stageObsClean`, `sumAbsStageErrorClean`, `avgStageErrorClean`) accumulates under the same gate as the stage bin write **and is suppressed when that write fails**, so `stageObsClean` equals the sum of the `stage_*` bins *by construction* rather than by assertion — a failed `stage_*` upsert previously incremented no counter at all and now routes into `binWriteFailures`/`lastBinError` like the CFS bin. `stageValidations` keeps its original semantics (every validation with a usable stage pair, hard-flagged or not): correct as a count — only the average was polluted — and v37.17's live diagnostics derive the null-stage total from it. Because it keeps incrementing, the frozen legacy average is now paired with a **latched** `legacyStageObs`/`legacyStageAvg` snapshot rather than the live counter, which would otherwise have displayed a frozen numerator against a rising denominator within one cron cycle. **The legacy fields are frozen, not reset and not deleted** — a cumulative sum, unlike an EMA, never washes out in place — following the v37.16 precedent for the retired NWS counters. **Correction to the first draft of this entry:** it justified freezing by claiming the clean history was *unrecoverable* because the bins store signed `sumError` rather than sum\|error\|. **That was false.** `validation_failure` rows are append-only with no retention and each carries `errorStage` (v37.9), so the history can be repaired exactly: clean n = 313 − 18 = **295** (which independently equals the measured stage-bin sum), clean sum = 26.21 − 2.18 = **24.03**, clean average **0.0815 ft** against the published 0.0837 — a **2.8% overstatement**. Freezing remains the shipped behaviour and the clean series still starts at n=0, because backfilling means writing to production metadata and that is an explicit operator decision, not something a cron path should do silently. **Expect the displayed stage error to move** as the clean series builds. 768 → 771 tests. |
| v37.18 | 2026-09-02 | **Dead ice-cleanup reset removed + two correctness fixes (TODO #28, #29) — MINOR. No model, learning, correction or estimate change.** Both defects were surfaced by adversarial verification of v37.17, not by v37.17 itself. **(1) The v24 low-flow ice-cleanup reset action deleted outright** (server action, client function, button, binding — 79 + 33 lines). Confirmed dead by the user, and *actively harmful* while it existed: it deleted only 12 of the 28 correction bins but upserted a `newMeta` literal that **replaces the whole metadata jsonb**, so surviving high-flow bins kept their observation counts while every counter restarted at 0 — permanently desynchronising the diagnostics panel from the bins, undetectably. A stale cached client that still POSTs it falls through to the handler's `result = { success: false }` default and reports a failed reset: it cannot delete anything. A structural guard test sweeps the whole source tree for the identifier (its first cut checked a hand-picked four files and missed a stale comment that then shipped into the published source map). **(2) `resetGFLearning` bin-write health, split rather than dropped.** That upsert replaces the whole jsonb, so every field not named in the literal is destroyed — and the bin-write health counters were unnamed, despite a `// Keep health stats` comment, so a learning reset silently zeroed the v37.7 diagnostics panel's entire subject. They are now handled by kind: **`binWriteFailures`/`lastBinError` survive** (a fault log must not be clearable by pressing reset — with the ice-cleanup action gone this is the only reset path), while **`binWriteSuccesses` resets with the bins it counts**, preserving v37.17's `sum(bin counts) == validValidations == binWriteSuccesses` reconciliation identity instead of leaving "307 ok" standing against zero bins forever. **(3) `errorStage` used truthiness** — `(predictedStage && actualStage)` — so a legitimate **0.00 ft** would be discarded as a missing measurement and v37.17's `stageSkipped` would have absorbed it. Now `Number.isFinite` on both operands, deliberately stricter than the `!= null` first drafted. **Correction to the first draft of this entry:** the justification named the wrong operand. `actualStage = lf.h` is assigned only inside `if (val > 0 && val < 9999999)` in the USGS parse, so from the gauge NaN *and* 0.00 are both unreachable. The genuinely exposed operand is `predictedStage` (`rawFinalStage`, a server-computed `estimateLFStage()` result), which is not range-gated — NaN is reachable there, the old truthiness test at least rejected it, and `!= null` would not have. The guard is right; the published reason was not. Both branches are locked by tests. 760 → 768 tests. |
...
*Last updated: 2026-09-02 (v37.19 — the headline stage error no longer averages over hard-flagged observations the correction bins already reject (n=313 in metadata vs 295 in the bins: 18 outliers inflating a published number — the v32.3 defect, never applied to the stage series). A clean series accumulates under the same gate as the stage bin write and is suppressed when that write fails, so its count equals the bin total by construction; a failed stage-bin upsert now registers in the bin-write health counters instead of only in the logs. The legacy fields are frozen as an audit trail, paired with a latched denominator. NB the clean history IS recoverable from the append-only `validation_failure` rows (repaired average 0.0815 ft vs the published 0.0837 — 2.8% overstatement); backfilling is an operator decision and has not been done. Expect the displayed stage error to move. No model, learning or estimate change.)*
