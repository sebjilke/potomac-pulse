...
**Version:** 37.0 | **Date:** June 2026 | **Full changelog:** [CHANGELOG.md](CHANGELOG.md)
...
**Current version:** v37.1 | **Last calibration:** v30.0 (Feb 2026) | **Last estimate-structural change:** v37.0 (Jun 2026 — C45: the applied correction is continuous in flow across the low/mid bin boundaries, not a hard step). v37.1 retired the legacy System-1 gauge travel-time learning (display-only; GF estimate unchanged).

For the complete version history (v16 through v37.1), see [CHANGELOG.md](CHANGELOG.md).

---

*Generated by Potomac Pulse v37.1 — System-1 (legacy client-side per-gauge travel-time learning) fully retired: display-only and MINOR, the GF estimate is unchanged. It had been a dead write path since the Feb-2026 modularization but still multiplied 15 frozen correction factors (mean 0.935) into the displayed per-gauge arrival times; those now equal baseHrs × Searcy-multiplier. Client `gauge-learning.js`/`cloud-sync.js`, the server `/api/sync` default handlers, and 43 stale DB rows removed; the only learning system is now System 2 (server-side GF EMA). Plan + audit: `analysis/system1-retirement-plan-2026-06-18.md`. 599 → 601 tests. — (v37.0: C45 flow-edge correction smoothing — MAJOR. The applied EMA correction is now CONTINUOUS in flow across the low/mid bin boundaries (3k/6k/12k): within ±12% flow of each, it ramps (log-flow) between the two bins' corrections instead of stepping; flows away from a boundary keep their exact binned correction. The 25000/50000 boundaries are deliberately LEFT as steps — at high flow the correction is genuine regime structure, and a prequential 14-yr backtest (110,548 obs, blind Python+R) showed smoothing them DEGRADED accuracy (the rejected full-width and all-boundary variants), whereas low/mid-only is accuracy-neutral-to-better (pooled MAE −1.1%, RMSE −0.6%; no bin worse than +1.1% median; high-flow bins exactly unchanged). The 18-bin EMA learning is unchanged (still keyed on the discrete raw-final bin), so there is no learn↔apply feedback. Because the corrected residual changes, the published headline-accuracy series has a discontinuity at the v36→v37 boundary (a methodology change, not a regression). Plan + gate: `analysis/c45-phase1-flow-interp-plan-2026-06-18.md`. 521 → 599 tests. (v36.4: server travel-time parity + PoR-history coverage, C8/C16. v36.3: documentation-accuracy sweep.))*
