...
| # | Item | Effort | Notes |
|---|------|--------|-------|
| 23 | 🔒 **Reset forecast accuracy — v37.16 IS DEPLOYED AND VERIFIED, so this is now unblocked** | 5 min | ✅ Gate verified live 2026-08-28 16:02Z (`+24h`: model 3044→3045 and persistence 1124→1125 incremented while `nwsRawValidations`/`nwsCorrectedValidations` stayed frozen at 1124 — only the v37.16 scorer splits them). **Blocked on the PIN only:** `ADMIN_PIN` is a Netlify env var and is not available locally, so the agent could not run it. Do NOT do it via direct SQL — that skips the PIN gate and the `audit_log` entry v37.8 added. ⚠ Every hour this waits, `gf_forecast_metadata` mixes old (mis-scored) and new (correct) validations, so the accuracy numbers are uninterpretable in the interim. Original notes follow. | **Required for the metric to mean anything.** Every `gf_forecast_metadata` counter accrued while forecasts were scored one GF→LF travel time too early; the counters are cumulative sums with no per-era split, so pre- and post-fix validations cannot be un-mixed later. Do it **after** confirming the new gate fired in production (not at deploy — if the deploy is broken you want the old numbers to compare against). No UI button exists (see #17): out-of-band PIN-gated POST `resetForecastAccuracy`, which also deletes pending forecast rows. Expect the accuracy panel to go dark for a while afterwards (`updateForecastAccuracyUI` hides below 10 validations; the +48h horizon needs ~2 days per validation). |
