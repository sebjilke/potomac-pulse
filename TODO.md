...
- ⚠️ **`scheduled-update` cron execution is intermittent — observed 2026-09-02.** Schedule is hourly (`netlify.toml:36`, `0 */1 * * *`), but `gf_metadata.lastPrediction` sat at **13:00:32Z** while the wall clock read **17:46Z** — 4 consecutive missed ticks — and `consecutiveRuns` had fallen **4 → 1**. **Not caused by that day's deploys:** the first push landed 15:04Z and the 14:00/15:00 ticks were already missed. The site itself was healthy throughout (client `gf_forecast_pending` writes at 12:52, 14:28 and 17:57Z), so this is specific to the scheduled function. Historical `missedRuns` = **548**, so intermittency has long precedent here and this may be the Netlify scheduler rather than anything new. **Not diagnosed** — the Netlify CLI is not installed locally and installing it was out of scope, so the function logs were never read. **Next step: check the `scheduled-update` function logs in the Netlify dashboard.** Blocks production verification of the v37.19/v37.20 cron paths (`legacyStageObs`, `stageObsClean`, `binReadFailures` were all still null at 17:46Z).
